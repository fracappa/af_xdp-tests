...
{
	return (size_t)jhash(key, sizeof(struct session_id), 0);
}

static bool equal_fn(const void *key1, const void *key2, void *ctx)
{
	const struct session_id *k1 = key1, *k2 = key2;
...
static void init_acl(const char *acl_path)
{
	...
	if (config.working_mode == MODE_AF_XDP) {
		hashmap__init(&acl, hash_fn, equal_fn, NULL);

		for (int i = 0; i < nrules; i++) {
			hashmap__set(&acl, &acl_entries[i].key, &acl_entries[i].value, NULL,
					NULL);
		}
	}
...
#endif
	hashmap__find(&acl, &key, (void **)&action);
#ifdef MONITOR_LOOKUP_TIME
