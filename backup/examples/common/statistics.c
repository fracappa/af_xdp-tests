...
struct xsknfv_socket_stats
		old_socket_stats[XSKNFV_MAX_WORKERS][XSKNFV_MAX_WORKERS];
struct xdp_cpu_stats old_xdp_stats[XSKNFV_MAX_WORKERS];
unsigned long old_total_xdp = 0;

static void compute_stats_per_second(struct xsknfv_socket_stats *current,
        struct xsknfv_socket_stats *prev, long dt,
...
static void acc_stats(struct xsknfv_socket_stats *acc,
        struct xsknfv_socket_stats *stats)
{
	unsigned *acc_arr = (unsigned *)acc, *stats_arr = (unsigned *)stats;
...
static void print_socket_stats(struct xsknfv_socket_stats *stats,
...
void dump_stats(struct xsknfv_config config, struct bpf_object *obj,
        int extra_stats, int app_stats)
{
	unsigned long now = get_nsecs();
	long dt = now - prev_time;
	struct xsknfv_socket_stats stats, total = {0};
...
void print_stats(struct xsknfv_config *config, struct bpf_object *obj)
{
