...
SEC("xdp") int ingress_load_balancer(struct xdp_md *ctx)
{
	...
	*dport = bkdinfo->port;
    ...
    newrep.dir = DIR_TO_BACKEND;
	newrep.addr = bkdinfo->addr;
	newrep.port = bkdinfo->port;
    __builtin_memcpy(&newrep.mac_addr, &bkdinfo->mac_addr,
            sizeof(newrep.mac_addr));
    newrep.ifindex = bkdinfo->ifindex;
	...
	newrep.dir = DIR_TO_CLIENT;
	newrep.addr = srvid.vaddr;
	newrep.port = srvid.vport;
    __builtin_memcpy(&newrep.mac_addr, &eth->h_source, sizeof(newrep.mac_addr));
    newrep.ifindex = ctx->ingress_ifindex;
